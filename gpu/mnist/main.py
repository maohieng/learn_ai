...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    return train_losses / len(train_loader.dataset)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tl = test(args, model, device, test_loader)
        test_losses.append(tl)
        scheduler.step()

    # Prepare exports files
    import os
    ...
    pd.DataFrame({'train_loss': train_losses, 'test_loss': test_losses}).to_csv(f'exports/losses_{suffix_filename}.csv', index=False)
